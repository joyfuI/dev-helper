...
#########################################################
	...
	def py_package_install(name):
		try:
			...
			output = subprocess.check_output([sys.executable, '-m', 'pip', 'install', '--upgrade', name], universal_newlines=True).rstrip()
			logger.debug(output)
			return output
		except Exception as e:
			logger.error('Exception:%s', e)
	...
	def py_package_uninstall(name):
		try:
			...
			output = subprocess.check_output([sys.executable, '-m', 'pip', 'uninstall', '-y', name], universal_newlines=True).rstrip()
			logger.debug(output)
			return output
		except Exception as e:
			logger.error('Exception:%s', e)
	...
	def ffmpeg_download(url, dir):
		...
		output = subprocess.check_output(['tar', 'xvfJ', filename, '-C', os.path.join(path_data, 'download_tmp')], universal_newlines=True)
		logger.debug(output)
		# 덮어쓰기
